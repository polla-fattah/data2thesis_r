...
**Teaching & Practice:** [Interactive Lecture Slides](../slides/14-advanced-clustering.html) · [Browser Playground Exercises](../playground/chapter14.html)
:::
...
The idea is easiest to see with one variable. R's `faithful` data records the waiting time, in minutes, between 272 eruptions of the Old Faithful geyser in Yellowstone National Park. The histogram in @fig-faithful-gmm has two humps: short waits and long waits. The **mclust** package fits a mixture model with `Mclust()`:
...
The function tried mixtures of one to nine groups and chose `r geyser$G`. The output describes them: the **mixing probabilities** are the sizes of the groups (about `r round(100 * gp$pro[1])`% and `r round(100 * gp$pro[2])`% of eruptions), the **means** are their centres (about `r round(gp$mean[1])` and `r round(gp$mean[2])` minutes), and the **variances** their spreads. @fig-faithful-gmm draws the two fitted normal curves over the data.

```{r}
#| label: fig-faithful-gmm
