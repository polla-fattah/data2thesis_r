...
Elaf chooses a local model, Google's Gemma 4 (the `e4b` version), for two reasons: her students' answers never leave her laptop, and anyone can repeat her analysis for free.

### Structured answers for many texts

For coding, Elaf needs two things that a chat on a website does not give: the same instructions for every answer, and a reply in a fixed format that R can use. The **system prompt** holds the instructions: the task and her codebook, with a definition of each theme. With an online model, a **type** describes the required answer: here, an object with one field, `theme`, which must be one of the seven themes:
...
Smaller local models do not always follow such a format. When Elaf tried `parallel_chat_structured()` with her local model, it answered correctly ("Finances", "Workload") but as plain words, not in the structured format, and ellmer could not read the replies. So she adds one line to the end of the codebook, "Reply with the name of the theme only", asks for plain text with `parallel_chat_text()`, and lets R check each reply against the list of themes:
...
Elaf's answers are anonymous, but she used a local model anyway, so they never left her laptop: the simplest way to stay within any consent form.
